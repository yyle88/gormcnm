...
// MaxStmt generates a SQL statement to retrieve the maximum value of the column, using 0 as the default value.
// MaxStmt 生成一个 SQL 语句，检索列的最大值，默认值为 0。
func (qs *CoalesceNonNullGuardian) MaxStmt(alias string) string {
	return qs.Stmt("MAX", "0", alias)
}

// MinStmt generates a SQL statement to retrieve the minimum value of the column, using 0 as the default value.
// MinStmt 生成一个 SQL 语句，检索列的最小值，默认值为 0。
func (qs *CoalesceNonNullGuardian) MinStmt(alias string) string {
	return qs.Stmt("MIN", "0", alias)
}

// AvgStmt generates a SQL statement to calculate the average value of the column, using 0 as the default value.
